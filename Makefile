...
GFORTRANVERSION = $(shell gfortran --version | grep -i fortran | sed 's/^.* //g' | sed 's/\..*//')
CFLAGS = -fno-automatic -ffixed-line-length-none 
# Additional compiler flag needed if gfortran version 10 is used
ifeq "$(GFORTRANVERSION)" "10"
...
RES=/home/krack/POWHEG-BOX-RES
else ifeq ("$(shell whoami)","nudzeim")
...
ifeq ("$(ANALYSIS)","LQ-xsec")
##To include Fastjet configuration uncomment the following lines. 
FASTJET_CONFIG=$(shell which fastjet-config)
LIBSFASTJET += $(shell $(FASTJET_CONFIG) --libs --plugins ) $(LCPP)
FJCXXFLAGS+= $(shell $(FASTJET_CONFIG) --cxxflags)
PWHGANAL=pwhg_analysis-LQ-xsec.o pwhg_bookhist-multi.o multi_plot.o smearmom.o
## Also add required Fastjet drivers to PWHGANAL (examples are reported)
PWHGANAL+= fastjetfortran.o
endif
