      ...
      integer j,jb,fl1,fl2,fl,leg,legi,legj,jres,kres,ii
...
c If zerowidth, it has to go through also the resonance
                  ...
                  if(.not.flg_QEDonly.and.is_coloured(fl)) then
                     if(kn_masses(leg).ne.0) then
                        ii = flst_born(leg,jb)
                        if(abs(ii) > 6) then
c    must be some BSM heavy coloured object
                           call set_colour(ii,colrep,'get')
                           if(colrep(1:1)=='3') then
c    3 or 3bar:  Same representation as quarks
                              ii=1
                           elseif(colrep(1:3) == 'adj') then
c    Same representation as gluons
                              ii=0
                           else
                              write(*,*) 'btildevirt: flavour ',ii,
     1                             ' has unknown colour representation ',colrep
                              write(*,*) 'exiting ...'
                              call pwhg_exit(-1)
                           endif
                        endif
...
c     c debug Luca: just put Iem = 0 for the this process, a better solution must be found
            Iem = 0d0
...
c$$$            endif
            ...
            if (flg_QEDonly) then
               if (Q.ne.0d0 .or. I.ne.0d0) then
                  write(*,*) 'Q and I should be zero '
     1                     //'if flg_QEDonly is true '
                  call pwhg_exit(-1)
               endif
            endif
...
c     the default include file is empty.
